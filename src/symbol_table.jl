...
mutable struct SymbolTable
    ...
    function SymbolTable(s::SymbolTable)
...
function define!(s::SymbolTable, name::String)
...
function define_builtin!(s::SymbolTable, name::String, index::Int)
    sym = MonkeySymbol(name, BuiltinScope, index, nothing)
    s.store[name] = sym
    return sym
end

function define_free!(s::SymbolTable, original::MonkeySymbol)
...
function define_outer!(s::SymbolTable, original::MonkeySymbol, level::Int)
...
function define_function!(s::SymbolTable, name::String)
    sym = MonkeySymbol(name, FunctionScope, 0, nothing)
    s.store[name] = sym
    return sym
end

function resolve(s::SymbolTable, name::String; level::Int = 0)
