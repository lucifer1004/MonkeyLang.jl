const BUILTINS = Pair{String, Builtin}["len" => Builtin(function (args::Vararg{Object}; env::Environment = Environment())
                                                            if length(args) != 1
...
get_builtin_by_name(name::String) = begin for (_name, builtin) in BUILTINS
    if name == _name
        return builtin
    end
end end
