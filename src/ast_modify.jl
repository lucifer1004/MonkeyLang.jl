...
modify(node::BlockStatement, modifier::Function) =
  BlockStatement(node.token, map(statement -> modify(statement, modifier), node.statements))

modify(node::PrefixExpression, modifier::Function) =
  PrefixExpression(node.token, node.operator, modify(node.right, modifier))

modify(node::InfixExpression, modifier::Function) =
  InfixExpression(node.token, modify(node.left, modifier), node.operator, modify(node.right, modifier))

modify(node::IfExpression, modifier::Function) =
  IfExpression(node.token, modify(node.condition, modifier), modify(node.consequence, modifier), modify(node.alternative, modifier))
...
modify(node::HashLiteral, modifier::Function) =
  HashLiteral(node.token, Dict(modify(key, modifier) => modify(value, modifier) for (key, value) in collect(node.pairs)))
