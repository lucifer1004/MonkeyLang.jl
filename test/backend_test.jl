function test_backend(run::Function, name::String; check_object::Bool = true)
    ...
    @testset "Empty program" begin @test isnothing(run("")) end

    @testset "Integer expression" begin for (code, expected) in [
        ("5", 5),
        ("10", 10),
        ("-5", -5),
        ("-10", -10),
    ...
    @testset "Boolean expression" begin for (code, expected) in [
        ("false", false),
        ("true", true),
        ("1 < 2", true),
        ("1 > 2", false),
    ...
    @testset "String expression" begin for (code, expected) in [
    ...
    @testset "Array" begin
        @testset "Literal" begin for (code, expected) in [
            ("[]", []),
            ("[1, 2, 3]", [1, 2, 3]),
            ("[1, 2 * 2, 3 + 3]", [1, 4, 6]),
        ]
        ...
        @testset "Index" begin for (code, expected) in [
            ("[1, 2, 3][0]", 1),
            ("[1, 2, 3][1]", 2),
            ("[1, 2, 3][2]", 3),
            ("let i = 0; [1][i]", 1),
    ...
    @testset "Hash" begin
        ...
        @testset "Index" begin
            ...
            @testset "Maybe broken" begin for (code, expected) in [
    ...
    @testset "Bang operator" begin for (code, expected) in [
        ("!true", false),
        ("!false", true),
        ("!5", false),
        ("!!true", true),
    ...
    @testset "If-else expression" begin for (code, expected) in [
        ("if (true) { 10 }", 10),
        ("if (false) { 10 }", nothing),
        ("if (null) { 2 } else { 3 }", 3),
        ("if (1) { 10 }", 10),
    ...
    @testset "While statement" begin
        @testset "Plain" begin for (code, expected) in [
        ...
        @testset "Nested" begin for (code, expected) in [("""
                                                          let a = 6;
                                                          let b = a;
                                                          let x = 1;
                                                          while (x > 0) {
        ...
        @testset "Inside functions" begin for (code, expected) in [
            ("""
             let f = fn(x) {
                 while (x > 0) {
                     if (x == 3) {
    ...
    @testset "Break statement" begin for (code, expected) in [("""
                                                               let a = 2;
                                                               let b = 0;
                                                               while (a > 0) {
                                                                   b = b + a;
    ...
    @testset "Continue statement" begin for (code, expected) in [("""
                                                                  let a = 4;
                                                                  let b = 0;
                                                                  while (a > 0) {
                                                                      a = a - 1;
    ...
    @testset "Return statement" begin for (code, expected) in [
        ("return 10;", 10),
        ("return 10; 9;", 10),
        ("return 2 * 5; 9;", 10),
        ("9; return 2 * 5; 9;", 10),
    ...
    @testset "Let statement" begin
        ...
        @testset "Reassign" begin for (code, expected) in [("let a = 5; a = a + 1; a;", 6)]
            check(code, expected)
        end end

        @testset "Redefine" begin for (code, expected) in [("""
                                                            let x = 5;
                                                            let y = 1;
                                                            let z = 0;
                                                            while (y > 0) {
    ...
    @testset "Function" begin
        ...
        @testset "Application" begin for (code, expected) in [
        ...
        @testset "Closure" begin for (code, expected) in [("""
                                                          let newAdder = fn(x) { 
                                                            fn(y) { x + y };
                                                          }
        ...
        @testset "Recursion" begin for (code, expected) in [("""
                                                             let fibonacci = fn(x) {
                                                             if (x == 0) {
                                                                 0
                                                             } else {
    ...
    @testset "Built-in functions" begin
        @testset "len" begin for (code, expected) in [
            ("len(\"\")", 0),
            ("len(\"four\")", 4),
            ("len([1, 2, 3])", 3),
            ("len([])", 0),
        ...
        @testset "first" begin for (code, expected) in [
            ("first([1, 2, 3])", 1),
            ("first([])", nothing),
            ("first(\"hello\")", "h"),
            ("first(\"\")", nothing),
        ...
        @testset "last" begin for (code, expected) in [
            ("last([1, 2, 3])", 3),
            ("last([])", nothing),
            ("last(\"hello\")", "o"),
            ("last(\"\")", nothing),
        ...
        @testset "rest" begin for (code, expected) in [
            ("rest([1, 2, 3])", [2, 3]),
            ("rest([1])", []),
            ("rest([])", nothing),
            ("rest(\"hello\")", "ello"),
        ...
        @testset "push" begin for (code, expected) in [
            ("push([], 2)[0]", 2),
            ("push({2: 3}, 4, 5)[4]", 5),
        ]
            check(code, expected)
        ...
        @testset "type" begin for (code, expected) in [
        ...
        @testset "puts" begin for (code, expected_output) in [
            ("puts()", ""),
            ("puts(1)", "1\n"),
            ("puts(\"hello\")", "hello\n"),
        ...
        @testset "Complex combination" begin for (code, expected) in [
            ("""
             let map = fn(arr, f) {
    ...
    @testset "Macro" begin for (code, expected) in [
        ("""
    ...
    @testset "Error handling" begin
        @testset "Invalid expression" begin
            ...
            @testset "Semantic analysis" begin for (code, expected) in [
        ...
        @testset "Built-in function" begin for (code, expected) in [
            ("len(1)",
